...
pub fn get_used_crates<'a>(cstore: &'a CStore, prefer: LinkagePreference)
    -> ~[(ast::CrateNum, Option<&'a Path>)]
{
    let mut ret = ~[];
    for src in cstore.used_crate_sources.iter() {
        ret.push((src.cnum, match prefer {
            RequireDynamic => src.dylib.as_ref(),
            RequireStatic => src.rlib.as_ref(),
        }));
    }
    return ret;
}
